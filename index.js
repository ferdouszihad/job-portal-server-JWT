...
async function run() {
  try {
    ...
    app.post("/jwt", async (req, res) => {
      const user = req.body;
      console.log(user);
      //                  payload  secret key           expireTime
      const token = jwt.sign(user, "secret-2441139", { expiresIn: "1h" });
      console.log(token);
      res.send({ token });
    });
    ...
    app.get("/jobs", async (req, res) => {
      ...
      if (email) {
        ...
        const today = new Date().toISOString(); //2024-12-12T12:50:41.076Z
        const currentDate = today.split("T")[0]; //2024-12-12
        query = { applicationDeadline: { $gte: currentDate } };
      }
